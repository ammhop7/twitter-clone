...
async def delete_tweet(db: AsyncSession, current_user: User, tweet_id: int):
...
async def like_tweet(db: AsyncSession, current_user: User, tweet_id: int):
    result = await db.execute(select(Tweet).where(Tweet.id == tweet_id))
    tweet = result.scalar_one_or_none()
    if not tweet:
        raise HTTPException(status_code=404, detail="Tweet not found")
...
async def unlike_tweet(db: AsyncSession, current_user: User, tweet_id: int):
    result = await db.execute(select(Tweet).where(Tweet.id == tweet_id))
    tweet = result.scalar_one_or_none()
    if not tweet:
        raise HTTPException(status_code=404, detail="Tweet not found")
...
async def get_tweets(db: AsyncSession, current_user: User):
    ...
    result = await db.execute(
        select(Tweet).where(Tweet.author_id.in_(following_ids)).order_by(desc(Tweet.id))
    )
